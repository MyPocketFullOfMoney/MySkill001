...
1. 调用类的构造函数，完成类的初始化;
2. 根据`__enter__()`方法返回对应的对象赋值给`as`后面的变量;
3. 执行with里面的代码;
4. 执行`__exit__()`方法，关闭对应的资源或者处理异常

[数据库上下文管理——类实现代码](代码/Python基本练习/DBConnection.py)

## 多线程

Python提供的标准库提供了两个模块来实现多线程：`_thread`和`threading`,`_thread`是低级模块，`threading`是高级模块，对`_thread`实现了封装。所以一般情况下我们都使用`threading`模块。

`threading`模块中的`Thread`类提供了以下方法来处理线程：

- run()：用以表示线程活动的方法。（多线程要处理的任务）。
- start()：启动线程活动。
- join([time])：等待至线程终止。这阻塞调用线程直至线程的join() 方法被调用中止-正常退出或者抛出未处理的异常-或者是可选的超时发生。
- isAlive()：返回线程是否活动。
- getName()：返回线程名称。
- setName()：设置线程名称。
...
2. `Thread`类直接创建对象，通过`target`参数，传入目标函数（多线程执行的函数）。
...
[廖雪峰Python教程——多线程](https://www.liaoxuefeng.com/wiki/1016959663602400/1017629247922688)
